...
La CPU intenta realizar También defender.'''
...
while not salir:
    ...
    # se añaden las lista de chequeo de 4 en raya
    lista_check_horiz = []
    i = 0
    for y in range(6):
        for x in range(4):
    ...
    empate = False
    turno = ra.devuelve_numero_random(0, 9)
    ...
    while not empate:
        ...
        jugador_cuatro_en_raya = ra.checkear_4enraya(lista_check_horiz, lista_check_vert, lista_cuadricula, 'O')
        cpu_cuatro_en_raya = ra.checkear_4enraya(lista_check_horiz, lista_check_vert, lista_cuadricula, 'X')

        # Comprobamos 4enRaya diagonales
        jugador_cuatro_en_diag = ra.checkear_diagonales(lista_check_diag,lista_cuadricula, 'O')
        cpu_cuatro_en_diag = ra.checkear_diagonales(lista_check_diag,lista_cuadricula, 'X')
        ...
        #comprobamos 4enRaya de la CPU y el empate
        ...
        if turno:
            if columna == -1:
                ...
                lista_sprites_adibujar.draw(pantalla)
                # implementamos el turno de la CPU
                if not turno:
                    cpu_col = ra.cpu_intenta_4raya(lista_check_horiz, lista_check_vert,
                    lista_check_diag, lista_cuadricula, lista_x100)
                    if cpu_col == -1:
                        cpu_col = ra.cpu_defiende(lista_check_horiz, lista_check_vert,
                        lista_check_diag, lista_cuadricula, lista_x100)
                        cpu = ra.Cpu(RES, lista_cuadricula, cpu_col)
            ...
            if turno:
                ...
                jugador = ra.Jugador() # nueva ficha
                lista_sprites_adibujar.add(jugador)
