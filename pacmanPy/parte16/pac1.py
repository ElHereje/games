...
class PacMan(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # colision con los puntitos, sirena y los items
        ...
        if self.check_colision_puntitos():
            ...
            if calculoSirena - self.ultimo_updateSirena > self.cadenciaSirena:
                ...
                # 1 - SONIDO AL COMER PUNTOS GORDOS
                if self.game.countDownAzules <= 0:
                    self.game.sonido_sirena.play(maxtime = 500)
                else: # .. si están en azul...
                    self.game.sonido_fantasmas_azules.play(maxtime = 500)

        # 7 - MODIFICAMOS LOS PTOS Y LA GESTIÓN DE SALIDA DE OTRO ITEM
        if self.check_colision_item():
            self.game.puntos += self.game.items.sumaPtos
            self.game.mostrarPuntosItem = 120  # (contador de tiempo de un mensaje)

            #7b - contamos el tiempo desde que se como el item hasta que sale el otro
    ...
    # Pacman se come los puntos gordos.
