...
class PacMan(pygame.sprite.Sprite):
    ...
    # 13 - modificamos la función para detectar quien es (id)
...
class Fantasma(pygame.sprite.Sprite):
    def __init__(self, game, centerx, centery, tipoFantasma, direccion):
        ...
        # 4 - añadimos la dirección del movimietno
        ...
        # 5 - añadimos las variables de las listas de los ojos
        self.cx = self.game.cx
        self.cy = self.game.cy

        # 6 - asignamos el elemento a cada valor de la lñista
        self.cx[(self.tipoF - 1) // 10] = self.rect.centerx
        self.cy[(self.tipoF - 1) // 10] = self.rect.centery

        # 7 - creamos un dicc con las direcciones alternativas
        self.hacia_donde = {
            'le' : 'doupri', 'ri' : 'updole',
            'up' : 'lerido', 'do' : 'leriup'
        }
        # 8 - otro para direccionar los hojos hacia pacman
        self.hacia_donde_velXY = {
            'le' : [-2, 0], 'ri' : [2, 0],
            'up' : [0, -2], 'do' : [0, 2]
        }
        # 9 - Identyificador a cada fantasma
    ...
    def update(self):
        ...
        # 11 - Damos movimiento a los fantasmas
        if self.rect.centerx % (self.game.BSX // 2) == 0 and self.rect.centery % (self.game.BSY // 2) == 0: # si es exacta
    ...
    # 12 - funcion para cambio de dirección de los dfantasmas (seleccionamos dos de las letras de las direcc)
