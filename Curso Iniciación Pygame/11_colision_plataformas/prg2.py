...
class Personaje(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # verificamos las colisiones con la fuinción creada abajo
    ...
    def leerTeclado(self):
        ...
        self.dy += self.game.GRAVEDAD # para poder caer si no hay plataforma
        self.rect.x += self.dx
        self.rect.y += self.dy # para poder caer si no hay plataforma
    ...
    # creamos otra funcion --> Colision para pletaformas:
    def checkColisionPlataformas(self): # (personaje, plataforma, no desaparece)
        colisiones = pygame.sprite.spritecollide(self, self.game.lista_plataformas, False)

        for colision in colisiones:
            # confirmamos que supera la plataforma en el salto:
            if self.rect.bottom <= colision.rect.bottom:
                # si bajamos tras un salto
                if self.dy > 0:
                    self.rect.bottom = colision.rect.top
                    self.dy = 0 # acaba es salto
                    self.saltar = False
...
class Plataforma(pygame.sprite.Sprite):
    ...
    def __init__(self, game, x, y, numero):
        ...
        if numero == 0:
            ...
            # los tiles --> la altura siempre será de 55 (distancia entre plataformas)
